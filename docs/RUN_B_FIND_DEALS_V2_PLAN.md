...
| Random site (glampingbusiness.com) has unusual DOM, returns 0 listings                                                                   | Medium     | This is a _valid_ outcome per Test Gate R criteria (zero is OK if `notes` field populated). Goal is graceful fail, not extraction success.                              |
