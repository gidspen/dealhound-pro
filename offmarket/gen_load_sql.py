...
DATA = json.load(open(os.path.join(ROOT, "data", "dental_targets.json")))
OUT = os.path.join(ROOT, "data", "sql")
os.makedirs(OUT, exist_ok=True)

NS = uuid.UUID("6f1d2c00-0000-4000-8000-000000000001")  # fixed namespace for this dataset
run = DATA["run"]
RUN_ID = str(uuid.uuid5(NS, "score_run:" + run["run_label"]))
...
def bid(b):
    key = f"{b.get('vertical','dental')}|{b['legal_name']}|{b.get('city','')}|{b.get('state','TX')}"
