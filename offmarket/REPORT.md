...
7. **Supabase IS written** — the autonomous sub-agent's `execute_sql` was permission-denied, but the main session loaded all data via the Supabase MCP: 60 businesses, 213 signals, 60 scores, `score_run_id = 38f59bde-c3a1-5f3a-8434-e0f6ef717462`. Query via `offmarket.scored_targets`. Reload is reproducible via `offmarket/gen_load_sql.py` (deterministic uuid5 keys) + ordered SQL chunks under `offmarket/data/sql/`.
